...
test_cases_gene_values = [
    ...
    (
        np.array(
            [("GeneA", 1.2, "extra")],
            dtype=[("Symbol", "U10"), ("Value", "f8"), ("Extra", "U10")],
        ),
        [GenesetValueInput(symbol="GeneA", value=1.2)],
    ),  # more than 2 columns
    ...
    (
        np.array([("GeneA",)], dtype=[("Symbol", "U10")]),
        ValueError("Input must be a 2-dimensional array"),
    ),  # less than 2 columns
]


@pytest.mark.parametrize(("geneset_array", "expected"), test_cases_gene_values)
def test_ndarray_to_gene_values(
    geneset_array: np.ndarray, expected: List[GenesetValueInput]
):
    """Test that the ndarray_to_gene_values function works as expected."""
